            ...
            try:
                with open(log_path, 'r', encoding='utf-8') as f:
                    ...
                    if content:  # 检查文件是否非空
                        ...
                        self._logger.debug(f"读取到 {len(log_data.get('recordings', []))} 条历史记录")
                    else:
                        self._logger.warning(f"日志文件为空，将创建新日志")
            except json.JSONDecodeError as e:
                self._logger.warning(f"日志文件 JSON 格式错误: {e}，将创建新日志")
                log_data = {
                    "date": self._today_str,
                    "channel_id": self._channel_id,
                    "recordings": []
                ...
                self._logger.warning(f"读取现有日志文件失败: {e}，将创建新日志")
                log_data = {
                    "date": self._today_str,
                    "channel_id": self._channel_id,
                    "recordings": []
