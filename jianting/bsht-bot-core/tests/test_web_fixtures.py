...
def test_db():
    ...
    # 初始化数据库（Database.__init__会自动调用_init_db）
...
def test_recording_data(test_db):
    ...
    test_recordings = [
        (1, "test1.opus", 1, "user1", "用户1", "RX", 5.2, now, 10240, 1, "测试内容1", "测试内容1", "CQ", 0.95, -20.0, 10.0),
        (2, "test2.opus", 1, "user1", "用户1", "RX", 3.1, now, 6144, 1, "测试内容2", "测试内容2", "CQ", 0.88, -18.0, 12.0),
        (3, "test3.opus", 1, "user2", "用户2", "RX", 8.5, now, 17408, 1, "测试内容3", "测试内容3", "QSO", 0.92, -22.0, 8.0),
    ]

    for recording in test_recordings:
        cursor.execute("""
            INSERT INTO recordings (id, filepath, channel_id, user_id, user_name, recorder_type,
