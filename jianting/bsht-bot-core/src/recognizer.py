...
class RecordingRecognizer:
        ...
        with self._lock:
            for task in self._pending_queue:
                if task['filepath'] == filepath:
                    logger.debug(f"已在处理队列中，跳过: {os.path.basename(filepath)}")
                    return
            # 检查是否正在处理
            if filepath in self._processing_files:
                logger.debug(f"正在处理中，跳过: {os.path.basename(filepath)}")
                return
    ...
    def _on_task_done(self, future: Future, task: dict):
        ...
        with self._lock:
            ...
            if filepath and filepath in self._processing_files:
    ...
    def _do_recognize(self, task: dict):
        ...
        if self._db:
            ...
            if existing and existing.recognition_status == "SUCCESS":
                ...
                self._print_result(ai_result, user_name, recorder_type,
                                  existing.asr_text or "", existing.recognize_duration or 0,
                                  start_time, duration=duration, lost_frames=lost_frames, loss_rate=loss_rate)
        ...
        try:
            ...
            if self._pusher and ai_result.success and ai_result.content:
                self._send_push(ai_result, user_name, channel_id, duration, start_time)
    ...
    def _send_push(self, ai_result, user_name: str, channel_id: int, duration: float, start_time: str = ""):
        ...
        try:
            ...
            # 格式化时间显示（音频抵达时间 - 完整日期时间）
            time_display = ""
            if start_time:
                try:
                    # start_time 格式: "2026-03-02 00:09:21.9" 或 "2026-03-02"
                    # 显示格式: "MM-DD HH:MM:SS"
                    if '.' in start_time:
                        # 有毫秒：2026-03-02 00:09:21.9
                        dt = datetime.fromisoformat(start_time)
                        time_display = dt.strftime("%m-%d %H:%M:%S")  # 月-日 时:分:秒
                    else:
                        # 无毫秒：2026-03-02
                        if len(start_time) >= 10:
                            time_display = f"{start_time[5:10]} {start_time[11:19]}"  # "03-02 00:09:21"
                        else:
...
━━━━━━━━━━━━━━━
⏰ 录音: {time_display} | ⏱️ 时长: {duration:.1f}s
🏷️ 类型: {ai_result.signal_type} | 📺 频道: {channel_id}"""
    ...
    def _print_result(self, ai_result, user_name, recorder_type,
        ...
        info_parts = []
        if duration > 0:
            info_parts.append(f"⏱️ 时长: {duration:.1f}s")
        if lost_frames > 0 or loss_rate > 0:
            info_parts.append(f"📉 丢包: {lost_frames}帧({loss_rate:.1f}%)")
        if info_parts:
    ...
    def scan_and_register_recordings(self, base_dir: str = "recordings", 
        ...
        # 确保 base_dir 是绝对路径
        base_dir = os_module.path.abspath(base_dir)
        ...
        for root, dirs, files in os.walk(base_dir):
            for filename in files:
                ...
                # 标准化路径
                filepath = os_module.path.normpath(filepath)
                ...
                if self._db:
                    ...
                    if existing:
                        ...
                        if existing.recognition_status == "SUCCESS":
                            ...
                            # 需要识别但还未识别，加入队列
                            logger.debug(f"[扫描] 未识别，加入队列: {filename}")
                ...
                if invalid_duration_threshold > 0 and duration > 0 and duration < invalid_duration_threshold:
                    ...
                    if self._db:
                        ...
                        if recognized_count >= max_count:
                            ...
                            break
            elif status == "needs_recognition":
                # 已在数据库中但未识别，直接触发识别
                logger.info(f"[扫描] 识别未完成: {filename} ({duration:.1f}s)")
                # 解析文件名获取用户信息
                parts = filename.replace('.wav', '').split('_')
                user_id = parts[1] if len(parts) >= 3 else "unknown"
                self.on_recording_complete(
